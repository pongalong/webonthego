	...
	$.fn.columnize = function(options) {
		...
		return this
				.each(function() {
					...
					/**
					 * return a node that has a height less than or equal to height
					 * 
					 * @param putInHere,
					 *          a dom element
					...
					function columnize($putInHere, $pullOutHere, $parentColumn, height) {
						...
						if ($item[0].nodeType == 3) {
							...
							if (oText.length) {
								...
								return false; // we ate the whole text node, move on to the next
															// node
							}
						}
					...
					function singleColumnizeIt() {
						...
						if (options.overflow) {
							...
							if (!$destroyable.children().find(":first-child").hasClass(
									"dontend")) {
					...
					function columnizeIt() {
						...
						$inBox.append($("<div style='width:"
								+ (Math.round(100 / numCols) - 2) + "%; padding: 3px; float: "
								+ options.float + ";'></div>")); // "
						...
						for ( var loopCount = 0; loopCount < maxLoops; loopCount++) {
							...
							for ( var i = 0; i < numCols; i++) {
								...
								var className = (i == numCols - 1) ? ("last " + className)
										: className;
								$inBox.append($("<div class='" + className + "' style='width:"
										+ (Math.round(100 / numCols) - 2) + "%; float: "
										+ options.float + ";'></div>")); // "
							}
							...
							while (i < numCols - (options.overflow ? 0 : 1)
									|| scrollHorizontally && $destroyable.children().length) {
								if ($inBox.children().length <= i) {
									// we ran out of columns, make another
									$inBox.append($("<div class='" + className
											+ "' style='width:" + (Math.round(100 / numCols) - 2)
											+ "%; float: " + options.float + ";'></div>")); // "
								}
							...
							if (options.overflow && !scrollHorizontally) {
								...
								if (IE6 || IE7) {
									...
									$(options.overflow.id).empty().append(
											$destroyable.children().clone(true));
								}
								...
								// it's scrolling horizontally, fix the width/classes of the
								// columns
