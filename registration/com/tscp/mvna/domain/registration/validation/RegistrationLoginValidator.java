...
import com.tscp.mvna.domain.registration.RegistrationLogin;
import com.tscp.mvna.form.ConfirmableFormField;

@Component
public class RegistrationLoginValidator implements Validator {
	@Autowired
	...
	public boolean supports(
			Class<?> myClass) {
		return RegistrationLogin.class.isAssignableFrom(myClass);
	}

	@Override
	public void validate(
			Object target, Errors errors) {
		RegistrationLogin registration = (RegistrationLogin) target;
		checkEmail(registration.getEmail(), errors);
		checkPassword(registration.getPassword(), errors);
	}

	protected void checkEmail(
			ConfirmableFormField email, Errors errors) {
	...
	protected static void checkPassword(
			ConfirmableFormField password, Errors errors) {
